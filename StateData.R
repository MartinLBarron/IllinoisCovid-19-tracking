#' Reads Johns Hopkins data and create a state-based comparison to IL
...
# Get Data (remote) ------------------------------------------------------------
states <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv")
IL <- read_csv("data/IL-covid-19-cases.csv") %>%
...
mostRecentDate <- max(states$Date)
...
states <- bind_rows(states, NY, Washington, California) %>%
  arrange(State, Date)
...
states <- bind_rows(states, IL)

todayDate <- format(IL[[nrow(IL), "Date"]],"%Y-%m-%d")
...
maxValue <- 10^ceiling(log10(max(states$Count)))
...
ggplot(data=states) +
  ...
  ggtitle("State-level Reported Cornoavirus Cases",
