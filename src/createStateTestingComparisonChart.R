createStateTestingComparisonChart <- function(stateData){
  
  
  #number days since 100th test
  df <- stateData %>%
    arrange(Date) %>%
    filter(totalTestResults>100) %>%
    group_by(state) %>%
  ...
  maxValue <- 10^ceiling(log10(max(df$totalTestResults)))
  ...
  gg <- ggplot(data=df) +
    geom_line(aes(x=daysSince100, y=totalTestResults, group=state, col=state), col="blue", size=2)+
    geom_point(aes(x=daysSince100, y=totalTestResults, group=state, col=state), col="blue", size=2)+
    ...
    ggtitle("Comparing IL to Other States Number of Cornoavirus Tests",
            subtitle="Cummulative number of tests, by number of days since 100th case") +
    ylab("Count of reported tests")+
